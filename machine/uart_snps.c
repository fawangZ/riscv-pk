...
#endif
#ifdef DDR4_1866
  // DDR-1866: clk to 466MHz
	REG_WRITE(CRU_BASE+0x50,0x000130af);//pll0_crt0 config
	REG_WRITE(CRU_BASE+0x54,0x00000000);//pll0_crt0 config
	REG_WRITE(CRU_BASE+0x58,0x00000300);//pll0_crt0 config
	REG_WRITE(CRU_BASE+0x5c,0x00000080);//pll0_crt0 config
  while (!(REG_READ(CRU_BASE+0x5c) & 0x10000)) {
  }
#endif
#ifdef DDR4_2133
  // DDR-2133: clk to 533MHz
	REG_WRITE(CRU_BASE+0x50,0x000130c8);//pll0_crt0 config
	REG_WRITE(CRU_BASE+0x54,0x00000000);//pll0_crt0 config
	REG_WRITE(CRU_BASE+0x58,0x00000300);//pll0_crt0 config
	REG_WRITE(CRU_BASE+0x5c,0x00000080);//pll0_crt0 config
  while (!(REG_READ(CRU_BASE+0x5c) & 0x10000)) {
...
void initUart()
{
    ...
    // REG_WRITE(FLL, 0xa2); // 0x00 25MHz/9600/16
    //REG_WRITE(DLL,0x44);   //0x00 125M/('h44*16) ~= 115200
    REG_WRITE(DLL,0x36);   //0x00 100M/('h36*16) ~= 115200
    // REG_WRITE(DLL,0x2b);   //0x00 80M/115200/16
    // REG_WRITE(DLL,0x20);   //0x00 60M/115200/16
...
DDRCmd cmds[] =
{
  ...
  {WRITE, MC,  0x31060064, 0x0040010B}, // RFSHTMG, tREFI, tRFC
  {WRITE, MC,  0x310600c0, 0x00000000},
  {WRITE, MC,  0x310600c4, 0x00001000},
  {WRITE, MC,  0x310600d0, 0x00020125},
  {WRITE, MC,  0x310600d4, 0x00760000},
  {WRITE, MC,  0x310600dc, 0x06350001}, // MR0, MR1, CL, tWR, tRTP
  {WRITE, MC,  0x310600e0, 0x00200200}, // MR2, MR3, CWL
  ...
  {WRITE, MC,  0x31060100, 0x11101112}, // DRAMTMG0, CWL, tWR, tFAW, tREFI, tRASmin
  {WRITE, MC,  0x31060104, 0x0004041A}, // DRAMTMG1, tXP, CL, tRASmin
  {WRITE, MC,  0x31060108, 0x0708040D}, // DRAMTMG2, CL, CWL, tWTR_L
  {WRITE, MC,  0x3106010c, 0x0000400C}, // DRAMTMG3, tMRD, tMOD
  {WRITE, MC,  0x31060110, 0x08030409}, // DRAMTMG4, CL, tCCD_L, tRRD_L
  {WRITE, MC,  0x31060114, 0x06060403}, // DRAMTMG5, tCKSRX, tCKSRE,  tCKE
  {WRITE, MC,  0x31060120, 0x06060C0A}, // DRAMTMG8, tRFC4, tRFC, tDLLK
  {WRITE, MC,  0x31060124, 0x0002030B}, // DRAMTMG9, tCCD_S, tRRD_S, CWL, tWTR_S
  {WRITE, MC,  0x31060128, 0x000E0C05}, // DRAMTMG10, tMOD
  {WRITE, MC,  0x3106012c, 0x1607010E}, // DRAMTMG11, tXS, tXSDLL, tMOD, tCPDED
  ...
  {WRITE, MC,  0x31060190, 0x048C820C}, // DFITMG0, CL, CWL
  ...
  {WRITE, PHY, 0x31070060, 0x00000635}, // MR0
  {WRITE, PHY, 0x31070061, 0x00000001}, // MR1
  {WRITE, PHY, 0x31070062, 0x00000020}, // MR2
  ...
  {WRITE, PHY, 0x31070044, 0x07241008}, // tRRD, tRAS, tRP, tRTP
  {WRITE, PHY, 0x31070045, 0x28201808}, // tWLMRD, tFAW, tMOD, tMRD
  {WRITE, PHY, 0x31070046, 0x00060264}, // tCKE, tXS
  {WRITE, PHY, 0x31070047, 0x03000101}, // tDLLK
  {WRITE, PHY, 0x31070048, 0x02580607}, // tRFC, tXP
  {WRITE, PHY, 0x31070049, 0x00341008}, // tRC, tRCD, tWTR
...
DDRCmd cmds[] =
{
  ...
  {WRITE, MC,  0x31060050, 0x00001070},
  {WRITE, MC,  0x31060060, 0x00000000},
  {WRITE, MC,  0x31060064, 0x003800EA}, // RFSHTMG, tREFI, tRFC
  {WRITE, MC,  0x310600c0, 0x00000000},
  {WRITE, MC,  0x310600c4, 0x00001000},
  {WRITE, MC,  0x310600d0, 0x00020125},
  {WRITE, MC,  0x310600d4, 0x00760000},
  {WRITE, MC,  0x310600dc, 0x04250001}, // MR0, MR1, CL, tWR, tRTP
  {WRITE, MC,  0x310600e0, 0x00180200}, // MR2, MR3, CWL
  {WRITE, MC,  0x310600e4, 0x00100000}, // MR4
  {WRITE, MC,  0x310600e8, 0x00000040}, // MR5
  {WRITE, MC,  0x310600ec, 0x00000400}, // MR6, tCCD_L
  {WRITE, MC,  0x310600f0, 0x00000010},
  {WRITE, MC,  0x31060100, 0x0F0E0F10}, // DRAMTMG0, CWL, tWR, tFAW, tREFI, tRASmin
  {WRITE, MC,  0x31060104, 0x00030417}, // DRAMTMG1, tXP, CL, tRASmin
  {WRITE, MC,  0x31060108, 0x0607040C}, // DRAMTMG2, CL, CWL, tWTR_L
  {WRITE, MC,  0x3106010c, 0x0000400C}, // DRAMTMG3, tMRD, tMOD
  {WRITE, MC,  0x31060110, 0x07030308}, // DRAMTMG4, CL, tCCD_L, tRRD_L
  {WRITE, MC,  0x31060114, 0x05050303}, // DRAMTMG5, tCKSRX, tCKSRE,  tCKE
  {WRITE, MC,  0x31060120, 0x06060A0A}, // DRAMTMG8, tRFC4, tRFC, tDLLK
  {WRITE, MC,  0x31060124, 0x0002030A}, // DRAMTMG9, tCCD_S, tRRD_S, CWL, tWTR_S
  {WRITE, MC,  0x31060128, 0x000E0C05}, // DRAMTMG10, tMOD
  ...
  {WRITE, MC,  0x31060180, 0x51000040},
  {WRITE, MC,  0x31060184, 0x00000070},
  {WRITE, MC,  0x31060190, 0x048A820A}, // DFITMG0, CL, CWL
  {WRITE, MC,  0x31060194, 0x01090606},
  {WRITE, MC,  0x31060198, 0x07008120},
  {WRITE, MC,  0x3106019c, 0x00000061},
  {WRITE, MC,  0x310601a0, 0x20400004},
  ...
  {WRITE, MC,  0x310601b0, 0x00000041},
  {WRITE, MC,  0x310601b8, 0x00000000},
  {WRITE, MC,  0x310601c0, 0x00000000},
  {WRITE, MC,  0x310601c4, 0x00000000},
  {WRITE, MC,  0x31060204, 0x003f1818}, // addr
  {WRITE, MC,  0x31060208, 0x00000000},
  {WRITE, MC,  0x3106020c, 0x00000000},
  ...
  {WRITE, PHY, 0x31070148, 0x0f000009},
  // {WRITE, PHY, 0x31070148, 0x0f090000},// 电压
  {WRITE, PHY, 0x310701a0, 0x00058e00},
  ...
  {WRITE, PHY, 0x31070344, 0x0e00003c},
  {WRITE, PHY, 0x31070384, 0x0e00003c},
  {WRITE, PHY, 0x31070080, 0x8000a0cf},
  {WRITE, PHY, 0x3107014b, 0x0fc00172},
  {WRITE, PHY, 0x31070040, 0x2800040c},
  {WRITE, PHY, 0x31070022, 0x20c01004},
  //{WRITE, PHY, 0x31070010, 0x5e001810},
  //{WRITE, PHY, 0x31070011, 0x008005f0},
  {WRITE, PHY, 0x31070010, 0xffffffff},
  {WRITE, PHY, 0x31070011, 0xffff9fff},
  {WRITE, PHY, 0x31070012, 0x000fffff},


  {WRITE, PHY, 0x31070060, 0x00000425}, // MR0
  {WRITE, PHY, 0x31070061, 0x00000001}, // MR1
  {WRITE, PHY, 0x31070062, 0x00000018}, // MR2
  {WRITE, PHY, 0x31070063, 0x00000200}, // MR3
  {WRITE, PHY, 0x31070064, 0x00000000}, // MR4
  {WRITE, PHY, 0x31070065, 0x00000040}, // MR5
  {WRITE, PHY, 0x31070066, 0x00000400}, // MR6
  {WRITE, PHY, 0x31070044, 0x06200E07}, // tRRD, tRAS, tRP, tRTP
  {WRITE, PHY, 0x31070045, 0x281C1808}, // tWLMRD, tFAW, tMOD, tMRD
  {WRITE, PHY, 0x31070046, 0x00050264}, // tCKE, tXS
  {WRITE, PHY, 0x31070047, 0x02550101}, // tDLLK
  {WRITE, PHY, 0x31070048, 0x02580606}, // tRFC, tXP
  {WRITE, PHY, 0x31070049, 0x002E0E07}, // tRC, tRCD, tWTR
  {WRITE, PHY, 0x31070005, 0x020046a0},
  {WRITE, PHY, 0x31070006, 0x20007025},
  {WRITE, PHY, 0x31070007, 0xc0aa0060},
  {WRITE, PHY, 0x3107000b, 0x80040000},
  {WRITE, PHY, 0x31070001, 0x00000073},

  //{WRITE, PHY, 0x310701a0, 0x0005ce00},
  //{WRITE, PHY, 0x310701a1, 0x1007bb00},
  ...
  //{WRITE, PHY, 0x31070094, 0x01044401},
  //{WRITE, PHY, 0x31070095, 0x10660101},
  // phy init
  {READ , PHY, 0x3107000d, 0x8000000f},
  // phy SDRAM config
  {WRITE, PHY, 0x31070001, 0x00040001},
  ...
  {WRITE, MC, 0x31060014, 0x0000048c},
  ...
  {WRITE, MC, 0x31060010, 0x00006010},
  {WRITE, MC, 0x31060014, 0x00000400},
  {WRITE, MC, 0x31060010, 0x80006010},
  {DEBUG_READ, MC, 0x31060010, 0x80001010},

  {WRITE, PHY, 0x310701c5, 0x0000002a},
  {WRITE, PHY, 0x31070205, 0x0000002a},
  {WRITE, PHY, 0x31070245, 0x0000002a},
  {WRITE, PHY, 0x31070285, 0x0000002a},
  {WRITE, PHY, 0x310702c5, 0x0000002a},
  {WRITE, PHY, 0x31070305, 0x0000002a},
  {WRITE, PHY, 0x31070345, 0x0000002a},
  {WRITE, PHY, 0x31070385, 0x0000002a},
 ...
 // write leveling training
  ...
  //{WRITE, PHY, 0x31070006, 0x00007025},
  //{WRITE, PHY, 0x31070080, 0x0000b0cf},
  ...
  //{WRITE, PHY, 0x3107014b, 0x1fc0f0f6},
   ...
   //{WRITE, PHY, 0x3107014b, 0x1fc0f094},
  ...
  // 20220221
  //{WRITE, PHY, 0x31070094, 0x00000001},
  //{WRITE, PHY, 0x310701c3, 0xf7fc0000},
  //{WRITE, PHY, 0x31070203, 0xf7fc0000},
  //{WRITE, PHY, 0x31070243, 0xf7fc0000},
  //{WRITE, PHY, 0x31070283, 0xf7fc0000},
